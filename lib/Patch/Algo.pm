...
use parent 'Exporter';
our @EXPORT_OK = qw(split_patch split_prepatch);
...
# split the given patch, remove the marked parts from it
# return the marked parts as a patch that can be applied before the given one
sub split_prepatch {
    my ($patch, $mark) = @_;
    ...
    if (ref $patch eq 'Patch') {
        ...
        for my $file ($patch->{files}->@*) {
            if (($file->{mark} // '') eq $mark) {
                ...
                my $new_file = split_prepatch($file, $mark);
                push @old_files, $file if $file->nonempty();
                push @new_files, $new_file if defined($new_file) && $new_file->nonempty();
            }
        }
        ...
        # the difference in line count because of hunk splitting
        # signifies the diff that goes into the 'next' patch, i.e. $self
        my $len_diff = 0;
        ...
        for my $hunk ($patch->{hunks}->@*) {
            if (($hunk->{mark} // '') eq $mark) {
                ...
                my $new_hunk = split_prepatch($hunk, $mark);
                $hunk->{old_start_line} += $len_diff;
                $hunk->{new_start_line} += $len_diff;
                $len_diff += $hunk->len_diff();
                push @old_hunks, $hunk if $hunk->nonempty();
                if (defined($new_hunk) && $new_hunk->nonempty()) {
        ...
        for my $line ($patch->{lines}->@*) {
            unless ($line->is_add() || $line->is_remove()) {
                ...
                push @old_lines, $line->normalize() unless $line->is_remove();
                push @new_lines, $line;
            } else {
                push @old_lines, $line;
                push @new_lines, $line->normalize if $line->is_remove();
            }
        }

        $patch->{old_start_line} = $patch->{new_start_line};
        $patch->{lines} = \@old_lines;
        return Patch::Hunk->new($patch->{old_start_line}, $patch->{new_start_line},
                                $patch->{context}, \@new_lines, $patch->{mark});
    }
}
